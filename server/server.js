...
app.get("/api/get_monthly_student_payments/:tutor_id", async (req, res) => {
  try {
    const { tutor_id } = req.params;

    console.log("Fetching payments for tutor:", tutor_id);

    // Call the PostgreSQL function
    const { data, error } = await supabase.rpc("get_monthly_student_payments", {
      current_tutor_id: tutor_id,
    });
